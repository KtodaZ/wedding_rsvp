        ...
        <div class="body-area" style="padding: 0;">
          <transition appear name="fadeRightBig" mode="out-in" leave-active-class="fadeOutLeft">
            ...
            <div v-else-if="attendee.name" key="codeSuccessHeader" class="legend-header">
              <div class="h1">Welcome, {{attendee.name}}!</div>
              ...
              <div class="h2" v-else-if="attendee.replied && !attending">
                ...
                <div>
                  <transition name="fadeRight" mode="out-in" leave-active-class="fadeOutLeft">
                    <div class="form-group" v-if="attending && attendee.replied">
                      <div class="Form">
                        <div class="Form-Header">Here are some links you might find useful:</div>
                        <div class="Form-container">
                          <p
                            v-if="attendee.eventContacts.length === 0 || (attendee.eventContacts.length === 1 && !attendee.eventContacts[0]['email'])"
                            class="norm-text"
                          >P.S. - We will email you this information if you enter your email (at the bottom of this page).</p>
                          <p
                            v-else
                            class="norm-text"
                          >P.S. - We sent you this list to your email. Please check your spam folder.</p>
                          ...
                          <p class="norm-text">
                            ...
                            <a
                              class="link"
                              target="_blank"
                              href="https://bit.ly/2LhKkVB"
                            >
                              Click
                              here
                            </a> to book with our reduced rate before July 5th!
                          </p>

                          <p class="norm-text">
                            Noelle and Kyle are registered at
                            <a
                              class="link"
                              target="_blank"
                              href="https://www.amazon.com/wedding/share/szombathyceane"
                            >Amazon</a>
                            and
                            <a
                              class="link"
                              target="_blank"
                              href="https://www.crateandbarrel.com/gift-registry/noelle-ceane-and-kyle-szombathy/r5840478"
                            >
                              Crate
                              & Barrel
                            </a>,
                            or you can help by donating to their
                            <a
                              class="link"
                              target="_blank"
                              href="https://www.pinterest.com/szombaceane/ceane-szombathy-newlywed-fund/"
                            >
                              Newlywed
                              Fund
                            </a>.
                  ...
                  <transition name="fadeRight" mode="out-in" leave-active-class="fadeOutLeft">
                    <div class="form-group" v-if="attendee.numAttending > 0 && attending">
                      <div class="Form">
                        <div v-if="attendee.replied && attending" class="Form-Header">
                          Edit your
                          email preferences? Here's what we have for you:
                        </div>
                        <div v-else class="Form-Header">
                          Enter one or more emails to receive
                          updates (optional)
                        </div>
                        <div class="Form-container">
                          <transition-group name="fadeRight" tag="p" leave-active-class>
                            <div
                              v-for="(contact, index) in attendee.eventContacts"
                              :key="'contact' + index"
                            >
                          ...
                          <div>
                            <span
                              class="Hint-clickable"
                              v-on:click="attendee.eventContacts.push({ email: '' })"
                  ...
                  <div class="form-group">
                    <div class="col-md-12 text-right">
                      <transition name="fadeRight">
                        <button
                          v-if="((attendee.numAttending > 0 && attending) || (attendee.numAttending === 0 && !attending)) && (attendee.numAttending != null && attending != null)"
                          type="submit"
                          class="Button"
                        >Submit</button>
                      </transition>
...
export default {
  ...
  methods: {
    onCodeSubmit: function() {
      ...
      axios
        .get(this.endpoint + "/code/" + this.code)
        .then(({ data }) => {
          this.handleSuccessfulResponse(data);
        })
        .catch(({ response }) => {
          this.errors = {
            0: "Hmmm. That code doesn't seem correct. Try checking the spelling one more time?"
          };
        });
    },
    onFormSubmit: function() {
      this.clearErrors();
      axios
        .put(this.endpoint + "/code/" + this.code, this.attendee)
        .then(({ data }) => {
          this.handleSuccessfulResponse(data);
        ...
        .catch(({ response }) => {
          this.errors = {
            0: "Something went wrong. Please try entering your code again."
          };
        });
    },
    ...
    setNumAttendingBtnColor() {
      ...
      $("#numAttendingBtn" + this.attendee.numAttending)
        .css("borderColor", "#f4b813")
        .css("color", "#f4b813");
    },
    clearNumAttendingButtons() {
      for (let n = 1; n <= this.attendee.numPlusOnesAllowed + 1; n++) {
        $("#numAttendingBtn" + n)
          .css("borderColor", "#3c4956")
          .css("color", "#3c4956");
      }
    },
    removeContact(n) {
